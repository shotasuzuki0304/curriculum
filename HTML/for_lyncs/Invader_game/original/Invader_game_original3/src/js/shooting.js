...
var BOSS_ENEMY_FIRE_INTERVAL = 10;
// ボス敵の発射インターバル
...
var moveBossEnemyBullets = function() {
    ...
    // 各弾ごとに処理を行う
    for(var i=0; i<BOSSENEMYBULLETS; i++) {
        // ヒットポイントを確認し、生きている場合のみ処理をする
        if(boss_enemy_bullets_hp[i] <= 0) {
            // ループの残りのステップを無視して次のループに行く場合は`continue` を指定する
            continue;
        ...
        if(boss_enemy_bullets_hp[i] <img_boss_enemy_bullets_hp.height) {
